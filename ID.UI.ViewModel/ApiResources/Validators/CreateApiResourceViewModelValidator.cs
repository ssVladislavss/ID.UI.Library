...
{
    public class CreateApiResourceViewModelValidator : AbstractValidator<CreateApiResourceViewModel>
    {
        public CreateApiResourceViewModelValidator()
        {
            RuleFor(x => x.Name)
        ...
        {
            var result = await ValidateAsync(ValidationContext<CreateApiResourceViewModel>
                .CreateWithOptions((CreateApiResourceViewModel)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
