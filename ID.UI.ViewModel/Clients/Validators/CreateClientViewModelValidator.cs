...
namespace ID.UI.ViewModel.Clients.Validators
{
    public class CreateClientViewModelValidator : AbstractValidator<CreateClientViewModel>
    {
        ...
        {
            var result = await ValidateAsync(ValidationContext<CreateClientViewModel>
                .CreateWithOptions((CreateClientViewModel)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
